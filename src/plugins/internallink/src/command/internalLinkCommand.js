...
import {
    CONFIG_HREF_KEY,
    CONFIG_TITLE_KEY,
    MODEL_INTERNAL_DATA, MODEL_INTERNAL_LINK_HREF_ATTRIBUTE,
...
export default class InternalLinkCommand extends Command {
        ...
        if (this.value !== newValue) {
            ...
            if (this.value) {
                new InternalLinkDataContext(this.editor).getTitleById(this.value)
                    .then(({data}) => {
                        const titleKey = this.editor.config.get(CONFIG_TITLE_KEY);

                        this.data = data.data || data
                        this.title = get(data.data || data, titleKey)
                    })
                    .catch(() => {
     ...
     */
    execute(internalLinkId, internalLinkText, internalLinkHref) {
        const model = this.editor.model;
