...
import { Plugin, ClickObserver, ContextualBalloon, clickOutsideHandler } from 'ckeditor5';
...
import {
    ...
    COMMAND_LINK,
    COMMAND_UNLINK,
    BUTTON_LINK, MODEL_INTERNAL_DATA, CONFIG_HREF_KEY
} from '../util/constants';
...
export default class InternalLinkUi extends Plugin {
    ...
    createFormView() {
        ...
        formView.titleInputView.bind('isEnabled').to(linkCommand, 'isEnabled');

        // Execute link command after clicking the "Save" button.
        this.listenTo(formView, 'submit', () => {
            const hrefKey = this.editor.config.get(CONFIG_HREF_KEY, null)

            editor.execute(
                COMMAND_LINK,
                formView.internallinkid,
                formView.title,
                formView.internalLinkData[hrefKey]
            );
 ...
 */
    addFormView() {
        ...
        this.formView.titleInputView.fieldView.select();
